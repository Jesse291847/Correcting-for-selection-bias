...
results <- parSim(
    ...
    # Conditions
    sample_size = c(500, 1000, 2500, 5000, 1000000),
    ...
    expression = {
      ...
      population <- as.data.frame(readRDS("objects/large_population_simulation.RDS"))
      true_network <- readRDS("network_sim.RDS")
        ...
        regular_multivariate <- multivariate(selection, min_sumscore = 0) #if this is to slow maybe we can do without it
